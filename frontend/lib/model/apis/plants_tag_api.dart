...
class PlantsTagApiService {
  ...
  Future<List<PlantTags>> fetchPlantsTagApi(var plantId) async {
    ...
    if (response.statusCode == 404) {
      ...
      throw CustomHttpException('404 Not Found',
    ...
    if (response.statusCode == 200) {
      ...
      //explore if this code is reachable
  ...
  Future<List<PlantTags>> fetchPlantsTagAccountApi(var accountId) async {
    final uri = Uri.parse(apiUrl + 'plantstagaccount?account_id=$accountId');
    final response = await client.get(uri).timeout(const Duration(seconds: 30));
  ...
  Future<List<PlantTags>> fetchRelatedPlantsApi(var tagId) async {
    final uri = Uri.parse(apiUrl + 'plantstagrelated?tag_id=$tagId');
    final response = await client.get(uri).timeout(const Duration(seconds: 30));
